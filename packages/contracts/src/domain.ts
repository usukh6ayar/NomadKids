...
/**
 * One "гэр бүлийн дурсамж" — RFP §4.3's family section, extended 2026-09-10 on
 * the client's instruction: who the child was with, what they did, when, and
 * the photograph of it.
 *
 * ★ The photograph is a `MediaFile` id, not an image embedded here.
 *
 * That is the whole reason a memory's picture also appears in the age album
 * without a second write: the upload goes through `POST /children/:id/media`
 * tagged `category=FAMILY` and `age=N` like any other album photo, and this
 * row only points at it. Storing a copy — a data URL, a second bucket object —
 * would give the album and the memory two pictures to disagree about, and the
 * album's own EXIF stripping and content sniffing would apply to only one.
 *
 * ★★ `mediaId` is nullish, and that is not laziness. A family with no
 * photograph of a moment still has the memory of it, and the section predates
 * photographs entirely: rows written before 2026-09-10 carry none.
 */
export const familyMemorySchema = z.object({
  /** Generated by the client — the list is one JSON column, not a table. */
  id: z.string(),
  mediaId: uuidSchema.nullish(),
  /** From `FAMILY_MEMBER_TYPES`, stored as the same plain strings as `familyMemberTypes`. */
  members: z.array(z.string()).default([]),
  title: z.string(),
  description: z.string().nullish(),
  /** `YYYY-MM-DD`. Nullish: a parent may not remember the day. */
