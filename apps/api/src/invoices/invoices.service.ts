...
export class InvoicesService {
  ...
  async generate(actor: Actor, kindergartenId: string, dto: GenerateInvoiceDto) {
    ...
    // ★ Every money field is forced through `.toFixed(2)` before it reaches
    // Prisma, not just the ones computed from arithmetic. A `Decimal` built
    // from a bare JS number carries no decimal places of its own — Postgres
    // still stores the column at its declared scale, but a sibling field
    // built from a padded string (like `totalDue` below) would round-trip
    // as "150000.00" while this one came back as "150000". Two fields on the
    // same invoice disagreeing about how many decimals a tögrög has is
    // exactly the kind of inconsistency the `money` convention exists to
    // prevent — CLAUDE.md's reasoning about a float applied to formatting
    // instead of precision.
    const baseAmount = sumLines(dto.lineItems, (t) => t === "TUITION").toFixed(2);
    const mealAmount = sumLines(dto.lineItems, (t) => t === "MEAL").toFixed(2);
    const extraAmount = sumLines(dto.lineItems, (t) => EXTRA_LINE_TYPES.has(t)).toFixed(2);
    ...
    const previousBalance = (previous ? previous.balance.toNumber() : 0).toFixed(2);
    ...
    if (existing) {
      ...
      const updated = await this.repo.replaceInvoice(
        existing.id,
        { baseAmount, mealAmount, extraAmount, discountAmount, previousBalance, totalDue, dueDate, note: dto.note ?? null, balance: totalDue, status: "UNPAID" },
        lineItems,
      );
...
function sumLines(
  ...
  return lineItems.filter((l) => matches(l.type)).reduce((sum, l) => sum + Number(l.amount), 0);
}
