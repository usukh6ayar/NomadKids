 ...
 *
 * ★★ **The "Бусад" pane is gone, and it was broken the whole time it existed.**
 *
 * Four of the five sections were marked `secondary`, kept out of the strip and
 * reached through a "Бусад" tab that rendered a grid of tiles linking to
 * `?tab=growth` and the rest. None of them could ever open. Radix decides which
 * `Tabs.Content` renders from the value on `Tabs.Root`, and this file set that
 * value to `"more"` whenever a secondary section was active — so following a
 * tile set `?tab=growth`, the root went back to `"more"`, and the tile grid
 * rendered again. A teacher pressing Өсөлт got Өсөлт's own tile back, forever.
 *
 * The strip's highlight and the panel selection were being driven by one value
 * that could only satisfy one of them.
 *
 * The indirection is removed rather than repaired because its premise had
 * already expired. It was introduced when this screen carried ten tabs ("ten
 * tabs is not a tab bar, it is a horizontal scroll nobody scrolls") — and the
 * child hub was split into separate pages afterwards, leaving five. Five fit,
 * and the strip below already scrolls on a phone. One value, one meaning, and
